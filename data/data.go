	...
	for _, player := range playersData {
		for _, fixture := range fixturesData {
			if (player.Team == fixture.HomeTeam) || (player.Team == fixture.AwayTeam) {
				...
				// TODO: train new models to work with new data format - but for now pre-process this data to work with legacy models.
				fixtureDifficulty = fixtureDifficulty * 2
				i, _ := strconv.Atoi(player.Influence)
				player.Influence = strconv.Itoa(i / 4)
				c, _ := strconv.Atoi(player.Creativity)
				player.Influence = strconv.Itoa(c / 4)
				t, _ := strconv.Atoi(player.Threat)
				player.Influence = strconv.Itoa(t / 4)
				ict, _ := strconv.Atoi(player.ICTIndex)
				player.Influence = strconv.Itoa(ict / 4)
...
// Candidates is a struct for storing the relevant data for selecting a team
...
func FilterCandidates(gameweek string) ([]Candidates, error) {
